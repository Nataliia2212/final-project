import{l as v,s as h,i as d}from"./assets/modal-feedback-0d03b697.js";import{a as x}from"./assets/vendor-4357aed0.js";const w="/final-project/assets/dumbbell-5f5def0b.png",o=document.querySelector(".favorites-list"),m=document.querySelector(".text-wrap"),u="favorites-exercises",y=` <img
        class="img"
        src="${w}"
        alt="dumbbell"
        width="85"
      ...
      </p>`;o.addEventListener("click",_);function _(e){if(e.target.dataset.action==="trash")E(e.target.id),p();else return}function E(e){const t=v(u),s=[];return t.map(i=>(i._id!==e&&s.push(i),s)),h(u,s)}function b(e){return e.map(({_id:t,bodyPart:s,name:i,target:n,burnedCalories:l,time:C})=>` <li class="list-item">
        <div class="wrap-hhhh">
...
<path d="M6.04894 0.927052C6.3483 0.0057416 7.6517 0.00574088 7.95106 0.927052L8.79611 3.52786C8.92999 3.93989 9.31394 4.21885 9.74717 4.21885H12.4818C13.4505 4.21885 13.8533 5.45846 13.0696 6.02786L10.8572 7.63525C10.5067 7.8899 10.3601 8.34127 10.494 8.75329L11.339 11.3541C11.6384 12.2754 10.5839 13.0415 9.80017 12.4721L7.58779 10.8647C7.2373 10.6101 6.7627 10.6101 6.41222 10.8647L4.19983 12.4721C3.41612 13.0415 2.36164 12.2754 2.66099 11.3541L3.50604 8.75329C3.63992 8.34127 3.49326 7.8899 3.14277 7.63525L0.930391 6.02787C0.146677 5.45846 0.549452 4.21885 1.51818 4.21885H4.25283C4.68606 4.21885 5.07001 3.93989 5.20389 3.52786L6.04894 0.927052Z" />
    ...
    <div class="modal-teg">
      ...
      </div>`}async function Z(){let e=localStorage.getItem("favorites-exercises"),t=e?JSON.parse(e):[];Array.isArray(t)||(t=[]);let s=await f.getExercisesById(`${r}`);if(a.modal_add_favorite.textContent==="Add to favorites"){const i={_id:s._id,bodyPart:s.bodyPart,equipment:s.equipment,gifUrl:s.gifUrl,name:s.name,target:s.target,description:s.description,rating:s.rating,burnedCalories:s.burnedCalories,time:s.time,popularity:s.popularity};t.push(i);const n=JSON.stringify(t);localStorage.setItem("favorites-exercises",n),a.modal_add_favorite.textContent="Remove from "}else a.modal_add_favorite.textContent="Add to favorites",D(r),p()}function D(e){const t=v(g),s=[];return t.map(i=>(i._id!==e&&s.push(i),s)),h(g,s)}const U=document.querySelector(".js-link-favor");U.classList.add("link-home-favor");const F=document.querySelector(".js-link-favor-mobail");F.classList.add("link-home-favor");
